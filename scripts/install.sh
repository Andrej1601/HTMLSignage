...
readonly SOURCE_WEBROOT="${PROJECT_ROOT}/webroot"
readonly SOURCE_CONFIG="${PROJECT_ROOT}/config"
readonly SOURCE_DEFAULT_DATA="${SOURCE_WEBROOT}/data"
...
require_command() {
  ...
  if ! command -v "$cmd" >/dev/null 2>&1; then
    error "Required command '$cmd' not found. Install package '$pkg' and retry."
    exit 1
  fi
}

verify_runtime_dependencies() {
  local deps=(python3 openssl rsync nginx)
  local dep
  for dep in "${deps[@]}"; do
    require_command "$dep"
  done
...
deploy_application() {
  ...
  if [[ ! -d $SOURCE_WEBROOT ]]; then
    error "Source webroot not found at ${SOURCE_WEBROOT}"
    exit 1
  fi
  ...
  rsync -a --delete --delete-delay \
    --filter='P .git/' \
    --exclude 'data/*.json' \
    "$SOURCE_WEBROOT"/ "$app_dir"/

  if [[ -d $SOURCE_DEFAULT_DATA ]]; then
    while IFS= read -r -d '' json; do
      local target="$app_dir/data/$(basename "$json")"
      if [[ ! -e $target ]]; then
        install -D -m 0644 "$json" "$target"
      fi
    done < <(find "$SOURCE_DEFAULT_DATA" -maxdepth 1 -type f -name '*.json' -print0)
  fi

  chown -R www-data:www-data "$app_dir"
  find "$app_dir" -path '*/.git' -prune -o -type d -print0 | xargs -0 chmod 2755
  find "$app_dir" -path '*/.git' -prune -o -type f -print0 | xargs -0 chmod 0644
}
...
configure_nginx() {
  ...
  install -m 0644 "$SOURCE_CONFIG"/nginx/signage-slideshow.conf /etc/nginx/sites-available/signage-slideshow.conf
  install -m 0644 "$SOURCE_CONFIG"/nginx/signage-admin.conf /etc/nginx/sites-available/signage-admin.conf
  install -m 0644 "$SOURCE_CONFIG"/nginx/snippets/signage-pairing.conf /etc/nginx/snippets/signage-pairing.conf
