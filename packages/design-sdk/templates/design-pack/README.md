# @htmlsignage/design-{{designId}}

{{designName}} — design pack for HTMLSignage.
...
3. Run `pnpm -F @htmlsignage/design-{{designId}} test` to verify the contract.
...
// ...
export type DesignId = 'wellness-classic' | '{{designId}}';
...
export const DESIGN_REGISTRY: Record<DesignId, () => Promise<Design>> = {
  ...
  '{{designId}}': () =>
    import('@htmlsignage/design-{{designId}}').then((m) => m.{{designIdCamel}}Design),
};
```
