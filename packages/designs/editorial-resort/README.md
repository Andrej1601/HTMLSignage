# @htmlsignage/design-editorial-resort

Editorial Resort — design pack for HTMLSignage.
...
3. Run `pnpm -F @htmlsignage/design-editorial-resort test` to verify the contract.
...
// ...
export type DesignId = 'wellness-classic' | 'editorial-resort';
...
export const DESIGN_REGISTRY: Record<DesignId, () => Promise<Design>> = {
  ...
  'editorial-resort': () =>
    import('@htmlsignage/design-editorial-resort').then((m) => m.editorialResortDesign),
};
```
...
```json
"@htmlsignage/design-editorial-resort": "workspace:*",
```
