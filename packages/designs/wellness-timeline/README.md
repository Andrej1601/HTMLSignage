# @htmlsignage/design-wellness-timeline

Wellness Timeline — design pack for HTMLSignage.
...
3. Run `pnpm -F @htmlsignage/design-wellness-timeline test` to verify the contract.
...
// ...
export type DesignId = 'wellness-classic' | 'wellness-timeline';
...
export const DESIGN_REGISTRY: Record<DesignId, () => Promise<Design>> = {
  ...
  'wellness-timeline': () =>
    import('@htmlsignage/design-wellness-timeline').then((m) => m.wellnessTimelineDesign),
};
```
...
```json
"@htmlsignage/design-wellness-timeline": "workspace:*",
```
