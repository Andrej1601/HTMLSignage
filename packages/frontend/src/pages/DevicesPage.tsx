...
import { useDevicesPageState, getFilterButtonClass } from '@/hooks/useDevicesPageState';
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-5 gap-4">
          <StatCard
            ...
            color="primary"
            details={[
              { label: 'Filter', value: state.activeFilterLabel, tone: 'neutral' },
            ]}
          />
          ...
          <StatCard title="Override-Modus" value={state.deviceStats.override} icon={ToggleRight} color="info" />
          ...
          <div className="space-y-4">
            <div className="relative max-w-xs">
              ...
              <input
                ...
                placeholder="Gerät suchen (Name, ID, Gruppe)..."
              ...
              <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
                <div>
                  ...
                  <p className="text-sm text-spa-text-secondary">
                    {state.visibleDevices.length} sichtbar in „{state.activeFilterLabel}", davon {state.selectedVisibleCount} in der aktuellen Auswahl.
                  </p>
                </div>
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <Button
                    ...
                    disabled={state.selectedDeviceIds.length === 0}
                  >
                    Auswahl löschen
                  </Button>
                </div>
                ...
                <div className="rounded-xl border border-spa-bg-secondary bg-white p-4">
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    <Button
                      ...
                      size="sm"
                      icon={RefreshCw}
                      onClick={() => state.openBulkCommand(
                        'reload',
                        'Geräte neu laden?',
                        `Die ausgewählten ${state.selectedDeviceIds.length} Geräte laden ihre Anzeigeinhalte neu.`,
                        'Neu laden',
                      )}
                      disabled={state.selectedDeviceIds.length === 0}
                    >
                    ...
                    <Button
                      ...
                      onClick={() => state.openBulkCommand(
                        'restart',
                        'Geräte neu starten?',
                        `Die ausgewählten ${state.selectedDeviceIds.length} Geräte erhalten einen Neustart-Befehl.`,
                        'Neustart senden',
                      )}
                      disabled={state.selectedDeviceIds.length === 0}
                    >
                    ...
                    <Button
                      ...
                      onClick={() => state.openBulkCommand(
                        ...
                        `Die ausgewählten ${state.selectedDeviceIds.length} Geräte leeren ihren lokalen Cache und laden danach neu.`,
                        'Cache leeren',
                      )}
                      disabled={state.selectedDeviceIds.length === 0}
                    >
                ...
                <div className="rounded-xl border border-spa-bg-secondary bg-white p-4">
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    <Button
                      ...
                      size="sm"
                      icon={ToggleRight}
                      onClick={() => state.openBulkUpdate(
                        { mode: 'auto' },
                        'Automatikmodus setzen?',
                        `Die ausgewählten ${state.selectedDeviceIds.length} Geräte wechseln gesammelt in den Automatikmodus.`,
                        'Automatik setzen',
                      )}
                      disabled={state.selectedDeviceIds.length === 0}
                    >
                    ...
                    <Button
                      ...
                      onClick={() => state.openBulkUpdate(
                        ...
                        `Die ausgewählten ${state.selectedDeviceIds.length} Geräte wechseln gesammelt in den Override-Modus.`,
                        'Override setzen',
                      )}
                      disabled={state.selectedDeviceIds.length === 0}
                    >
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    <Button
                      ...
                      onClick={() => state.openBulkUpdate(
                        ...
                        `Die ausgewählten ${state.selectedDeviceIds.length} Geräte werden gesammelt für Wartung und Rollouts markiert.`,
                        'Wartung aktivieren',
                        'warning',
                      )}
                      disabled={state.selectedDeviceIds.length === 0}
                    ...
                    <Button
                      ...
                      onClick={() => state.openBulkUpdate(
                        ...
                        `Die ausgewählten ${state.selectedDeviceIds.length} Geräte verlassen gesammelt den Wartungsmodus.`,
                        'Wartung beenden',
                      )}
                      disabled={state.selectedDeviceIds.length === 0}
                    >
        ...
        <SectionCard
          ...
          description={state.activeGroupFilter === 'all'
            ? 'Alle verbundenen Geräte im Überblick.'
            : `Gefiltert nach ${state.activeFilterLabel}.`}
          icon={Monitor}
        >
