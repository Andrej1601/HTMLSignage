...
:root{
  ...
  --cutTop:28%;
  --cutBottom:12%;
  --tileVW:45;         /* Prozent der Bildschirmbreite */
  --tileMinPx:480px;   /* Min Kachelbreite */
  --tileMaxPx:1100px;  /* Max Kachelbreite */
}
*{box-sizing:border-box}
html,body{height:100%;margin:0;background:var(--bg);color:var(--fg);font-family:var(--font)}
.slideshow{overflow:hidden}

/* 16:9 Canvas – ohne transform, immer zentriert */
.fitbox{position:fixed; inset:0; display:grid; place-items:center; background:var(--bg); height:100svh; min-height:100vh}
.canvas{position:relative; width:min(100vw, calc(100svh * (16/9))); height:min(100svh, calc(100vw * (9/16))); aspect-ratio:16/9}
...
/* Layout & Typo */
.container{position:relative; height:100%; padding:5% 3%; display:flex; flex-direction:column; align-items:flex-start}
.container.has-right{padding-right:calc(var(--rightW) + 3%)}
.container.overview{padding-right:3%}
.h1{font-weight:800;letter-spacing:.02em;font-size:calc(50px*var(--scale));margin:0 0 10px}
.h2{font-weight:700;letter-spacing:.01em;opacity:.95;font-size:calc(28px*var(--scale));margin:0 0 14px}
.caption{opacity:.8;font-size:calc(20px*var(--scale))}

/* Freier Bereich unter den Überschriften */
.body{flex:1; display:flex}
.list{display:flex;flex-direction:column;gap:1.4vh;width:100%;align-items:flex-start;justify-content:center}

/* Rechte Bildspalte – steiler/parametrisierbar */
.rightPanel{
...
.grid thead th{font-size:calc(20px*var(--scale));padding:8px 12px;text-align:center;background:transparent;color:var(--fg)}
.grid tbody tr:nth-child(odd) td{background:var(--zebra1)}
.grid tbody tr:nth-child(even) td{background:var(--zebra2)}
.grid td{font-size:calc(22px*var(--scale));padding:8px 12px}
.grid .timecol{background:var(--timecol)!important;text-align:center;font-weight:800;min-width:10ch;color:var(--fg)}
.cellwrap{display:flex;align-items:center;justify-content:space-between;gap:16px;width:100%}
...
.ovfit{transform-origin:top left}

/* Sauna-Liste – Breite: clamp(min, vw-basiert, max); Flammen rechts */
.tile{display:grid; grid-template-columns:1fr auto; align-items:center; gap:0.9vw;
  width: clamp(var(--tileMinPx), calc(var(--tileVW) * 1vw), var(--tileMaxPx));
  padding:1.2vh 1.2vw; background:var(--cell); border:2px solid var(--grid); border-radius:12px; color:var(--boxfg);
}
.title{font-size:calc(40px*var(--scale));font-weight:700}
.flames{display:flex;gap:10px;align-items:center; justify-self:end}
.flame{width:calc(28px*var(--scale));height:calc(28px*var(--scale))}
...
(() => {
  ...
  function applyLayout(){
    // Rechte Spalte Breite & Schrägschnitt
    ...
    // Kachelbreite (% vw) + Min/Max px
    const s = settings?.slides || {};
    const tile = (typeof s.tileWidthPercent === 'number') ? s.tileWidthPercent : 45;
    const tmin = (typeof s.tileMinPx === 'number') ? s.tileMinPx : 480;
    const tmax = (typeof s.tileMaxPx === 'number') ? s.tileMaxPx : 1100;
  ...
  function flamesWrap(spec){
    ...
    if (!spec) count = 0; else if (spec==='1') count=1; else if (spec==='2') count=2; else if (spec==='3') count=3;
    else if (spec==='1-2') { count=2; approx=true; } else if (spec==='2-3' || spec==='1-3') { count=3; approx=true; }
  ...
  function tableGrid(){
    ...
    for (const row of schedule.rows) {
      ...
      row.entries.forEach((cell) => {
        const td = h('td', {}, []);
        if (cell && cell.title) {
          const wrap = h('div', {class:'cellwrap'}, [ h('span', {class:'chip'}, cell.title.replace(/\*+$/,'')), flamesWrap(cell.flames||'') ]);
          td.appendChild(wrap);
    ...
    const hasStar = schedule.rows.some(r => (r.entries||[]).some(c => c && /\*$/.test(c.title||'')));
    if (hasStar && settings?.footnote) return h('div', {}, [t, h('div', {class:'footer-note'}, settings.footnote)]);
    return t;
  }
  ...
  function fitOverview(container){
    ...
    const ov = container.querySelector('.ovfit'); if (!ov) return;
    const avail = container.clientHeight - headH - 16;
    ov.style.transform = 'scale(1)';
    const actual = ov.getBoundingClientRect().height;
    const s = Math.min(1, avail / Math.max(1, actual));
    ov.style.transform = `scale(${s})`;
  }

  function renderOverview(){
    const table = tableGrid();
    const c = h('div', {class:'container overview fade show'}, [ h('h1', {class:'h1'}, 'Aufgusszeiten Übersicht'), h('div', {class:'ovfit'}, [table]) ]);
  ...
  function renderSauna(name){
    ...
    for (const it of items) {
      ...
      const tile = h('div', {class:'tile'}, [ h('div', {class:'title'}, label), flamesWrap(it.flames) ]);
  ...
  async function bootstrap(){
    ...
    setInterval(async ()=>{
      ...
      try { const cf = await loadJSON('/data/settings.json'); if (cf.version !== lastSetVer) { settings=cf; lastSetVer=cf.version; applyTheme(); applyLayout(); } } catch(e){}
    }, 3000);
  }
  bootstrap();
})();
...
<head>
  ...
  <style>
    :root{
      --bg:#070a12; --fg:#e8eeff; --mut:#9aa3b7; --acc:#E7B416; --acc2:#ffd560;
      --br:#21314e; --card:#0c1220; --card2:#0a0f1a; --input:#0e1426; --inbr:#2a3a5c;
    ...
    *{box-sizing:border-box}
    ...
    .btn.primary{background:var(--acc);color:#0b0d12;border-color:var(--acc);font-weight:700}
    .btn.ghost{background:transparent}
    .btn.sm{padding:6px 10px;border-radius:10px}

    main.layout{width:100%;display:grid;grid-template-columns:minmax(0,1fr) 440px;gap:16px;padding:16px 12px 18px 16px;align-items:start}
    .leftcol{display:flex;flex-direction:column;gap:16px;min-width:0}
    .rightbar{position:sticky;top:64px;max-height:calc(100svh - 64px);overflow:auto;padding-right:6px}
    ...
    .rgrid{display:grid;grid-template-columns: 1fr 64px 1fr auto auto;gap:10px;align-items:center}
...
<body>
    ...
    <aside class="rightbar">
      <details class="ac" open>
        ...
        <div class="content">
          ...
          <div class="kv"><label>Schrift-Skalierung</label><input id="fontScale" class="input" type="number" step="0.05" min="0.5" max="2" value="1"></div>
      ...
      <details class="ac" open>
        <summary>
          <div class="ttl">▶<span class="chev">⮞</span> Layout</div>
          <div class="actions"><button class="btn sm ghost" id="resetLayout">Standardwerte</button></div>
        </summary>
        <div class="content">
          <div class="kv"><label>16:9 Fit</label><select id="fitMode" class="input"><option value="width">volle Breite</option><option value="contain">komplett einpassen</option></select></div>
          <div class="kv"><label>Kachelbreite (%)</label><input id="tileW" class="input" type="number" min="10" max="100" value="45"></div>
          <div class="kv"><label>Min-Kachelbreite (px)</label><input id="tileMin" class="input" type="number" min="100" max="4000" value="480"></div>
          <div class="kv"><label>Max-Kachelbreite (px)</label><input id="tileMax" class="input" type="number" min="200" max="8000" value="1100"></div>
          <div class="kv"><label>Rechts-Breite (%)</label><input id="rightW" class="input" type="number" min="10" max="70" value="38"></div>
          <div class="kv"><label>Schräg oben (%)</label><input id="cutTop" class="input" type="number" min="0" max="60" value="28"></div>
          <div class="kv"><label>Schräg unten (%)</label><input id="cutBottom" class="input" type="number" min="0" max="60" value="12"></div>
        </div>
      </details>

      <details class="ac" open>
        <summary>
          <div class="ttl">▶<span class="chev">⮞</span> Hervorhebung „nächster Aufguss“</div>
        </summary>
        <div class="content">
          <div class="kv"><label>Aktiviert</label><input id="hlEnabled" type="checkbox"></div>
      ...
      <details class="ac" open>
        <summary>
          <div class="ttl">▶<span class="chev">⮞</span> Flammen-Bild</div>
          <div class="actions"><button class="btn sm ghost" id="resetFlame">Zurücksetzen</button></div>
        </summary>
        <div class="content">
          <div class="row">
            <img id="flamePrev" class="prev" alt="">
            <input id="flameImg" class="input" type="url" placeholder="/assets/img/flame_test.svg" style="flex:1 1 auto" />
      ...
      <details class="ac" open>
        <summary>
          <div class="ttl">▶<span class="chev">⮞</span> Rechtsbilder je Sauna</div>
        </summary>
        <div class="content">
          <div class="rgrid rhead" style="opacity:.7;font-weight:700;margin-bottom:6px">
            <div>Sauna</div><div>Preview</div><div>URL</div><div>Upload</div><div>Default</div>
          </div>
          <div id="rightImgs" class="rgrid"></div>
          <div class="row" style="margin-top:10px;justify-content:space-between">
            <small style="opacity:.7">Uploads landen unter <code>/assets/img/</code>. PNG/JPG/WebP/SVG werden unterstützt.</small>
            <button class="btn sm" id="btnCleanup">Assets aufräumen</button>
          </div>
        </div>
      </details>
      ...
      <details class="ac">
        <summary>
          <div class="ttl">▶<span class="chev">⮞</span> Farben (Hex)</div>
      ...
      <details class="ac">
        <summary>
          <div class="ttl">▶<span class="chev">⮞</span> Saunen (Namen)</div>
        </summary>
        <div class="content" id="saunaList"></div>
      </details>
    </aside>
  </main>
  ...
  <script>
    ...
    const asInt=(v,def)=>{ v=Number(v); return Number.isFinite(v)?v:def; };
    ...
    async function loadAll(){
      ...
      schedule=s; settings=cfg; renderGrid(); renderSettings(); renderColors(); renderSaunaNames(); renderRightImgs();
    }
    ...
    function renderGrid(){
      ...
      html+='</tbody>';
      $('#grid').innerHTML=html;
      $$('#grid .time input').forEach(inp=>{ inp.onchange=()=>{ const ri=+inp.parentElement.dataset.ri; const t=parseTime(inp.value); if(!t){ alert('Bitte HH:MM'); inp.value=schedule.rows[ri].time; return;} schedule.rows[ri].time=t; schedule.rows.sort((a,b)=>a.time.localeCompare(b.time)); renderGrid(); }; inp.onclick=()=>{ curRow=+inp.parentElement.dataset.ri; updateSelTime(); }; });
      $$('#grid .cellbtn').forEach(btn=>{ btn.onclick=()=>{ curRow=+btn.dataset.ri; curCol=+btn.dataset.ci; updateSelTime(); const cell=schedule.rows[curRow].entries[curCol]; $('#m_time').value=schedule.rows[curRow].time; $('#m_title').value=cell?.title||''; $('#m_flames').value=cell?.flames||''; $('#modal').style.display='grid'; $('#m_title').focus(); }; });
    }
    ...
    function renderSettings(){
      // slides
      $('#overviewSec').value = settings.slides?.overviewDurationSec ?? DEFAULTS.slides.overviewDurationSec;
      $('#saunaSec').value    = settings.slides?.saunaDurationSec ?? DEFAULTS.slides.saunaDurationSec;
      $('#transMs').value     = settings.slides?.transitionMs ?? DEFAULTS.slides.transitionMs;
      $('#fontScale').value   = settings.fonts?.scale ?? 1;
      $('#footnote').value    = settings.footnote ?? '';

      // layout
      $('#fitMode').value     = settings.display?.fit || DEFAULTS.display.fit;
      $('#tileW').value       = settings.slides?.tileWidthPercent ?? DEFAULTS.slides.tileWidthPercent;
      $('#tileMin').value     = settings.slides?.tileMinPx ?? DEFAULTS.slides.tileMinPx;
      $('#tileMax').value     = settings.slides?.tileMaxPx ?? DEFAULTS.slides.tileMaxPx;
      $('#rightW').value      = settings.display?.rightWidthPercent ?? DEFAULTS.display.rightWidthPercent;
      $('#cutTop').value      = settings.display?.cutTopPercent ?? DEFAULTS.display.cutTopPercent;
      $('#cutBottom').value   = settings.display?.cutBottomPercent ?? DEFAULTS.display.cutBottomPercent;
      ...
      $('#hlAfter').value     = Number.isFinite(+hl.minutesAfter)?hl.minutesAfter:DEFAULTS.highlightNext.minutesAfter;
      const setSw=()=> $('#hlSw').style.background = $('#hlColor').value; setSw(); $('#hlColor').addEventListener('input',()=>{ if(/^#([0-9A-Fa-f]{6})$/.test($('#hlColor').value)) setSw(); });
      ...
      // reset handlers
      $('#resetSlides').onclick = ()=>{ $('#overviewSec').value=DEFAULTS.slides.overviewDurationSec; $('#saunaSec').value=DEFAULTS.slides.saunaDurationSec; $('#transMs').value=DEFAULTS.slides.transitionMs; $('#fontScale').value=1; $('#footnote').value=''; };
      $('#resetLayout').onclick = ()=>{ $('#fitMode').value=DEFAULTS.display.fit; $('#tileW').value=DEFAULTS.slides.tileWidthPercent; $('#tileMin').value=DEFAULTS.slides.tileMinPx; $('#tileMax').value=DEFAULTS.slides.tileMaxPx; $('#rightW').value=DEFAULTS.display.rightWidthPercent; $('#cutTop').value=DEFAULTS.display.cutTopPercent; $('#cutBottom').value=DEFAULTS.display.cutBottomPercent; };
      $('#resetFlame').onclick  = ()=>{ $('#flameImg').value=DEFAULTS.assets.flameImage; updateFlamePreview($('#flameImg').value); settings.assets=settings.assets||{}; settings.assets.flameImage=$('#flameImg').value; };

      // uploads
      $('#flameImg').onchange  = ()=>{ const u=sanitizeUrl($('#flameImg').value); if(!u){ alert('Ungültige URL'); return;} updateFlamePreview(u); settings.assets=settings.assets||{}; settings.assets.flameImage=u; };
      $('#flameFile').onchange = ()=> uploadGeneric($('#flameFile'), (p)=>{ $('#flameImg').value=p; updateFlamePreview(p); settings.assets=settings.assets||{}; settings.assets.flameImage=p; });
    }

    function renderColors(){
      const host=$('#colorList'); host.innerHTML='';
      const theme=settings.theme||{};
      const fields=[ ['bg','Hintergrund'],['fg','Vordergrund/Schrift'],['accent','Akzent'],['gridBorder','Tabellenrand'],['cellBg','Zellen-Hintergrund'],['boxFg','Box-Schrift'],['saunaColor','Sauna-Überschrift'],['timeColBg','Zeitspalten-Hintergrund'],['flame','Flammen'],['zebra1','Zebra 1'],['zebra2','Zebra 2'] ];
      fields.forEach(([key,label])=>{
        const row=document.createElement('div'); row.className='kv';
        row.innerHTML=`<label>${label}</label><div class="color-item"><div class="swatch" id="sw_${key}"></div><input class="input" id="cl_${key}" type="text" value="${theme[key]||DEFAULTS.theme[key]}" placeholder="#RRGGBB" pattern="^#([0-9A-Fa-f]{6})$"></div>`;
        host.appendChild(row);
      });
      fields.forEach(([key])=>{ const inp=$(`#cl_${key}`), sw=$(`#sw_${key}`); const setPrev=v=>sw.style.background=v; setPrev(inp.value); inp.addEventListener('input',()=>{ if(/^#([0-9A-Fa-f]{6})$/.test(inp.value)) setPrev(inp.value); }); });
      $('#resetColors').onclick=()=>{ fields.forEach(([key])=>{ $(`#cl_${key}`).value=DEFAULTS.theme[key]; $(`#sw_${key}`).style.background=DEFAULTS.theme[key]; }); };
    }

    function renderSaunaNames(){
      const cont=$('#saunaList'); cont.innerHTML='';
      (schedule.saunas||[]).forEach((name,i)=>{ const row=document.createElement('div'); row.className='kv'; row.innerHTML=`<label>${i+1}.</label><input class="input" type="text" value="${name}" data-i="${i}">`; cont.appendChild(row); });
      $$('#saunaList input').forEach(inp=>{ inp.onchange=()=>{ const i=+inp.dataset.i; schedule.saunas[i]=inp.value.trim(); renderGrid(); renderRightImgs(); }; });
    }

    function rightImgRow(name, val){
      const id='ri_'+btoa(unescape(encodeURIComponent(name))).replace(/[^A-Za-z0-9]/g,'');
      const wrap=document.createElement('div'); wrap.className='rgrid';
      wrap.innerHTML=`
        <label>${name}</label>
        <img id="p_${id}" class="prev" alt=""/>
        <input id="u_${id}" class="input" type="url" placeholder="/assets/img/${name.replace(/\s+/g,'_').toLowerCase()}.png" value="${val||''}" />
      ...
      `;
      const $inp=$(`#u_${id}`,wrap), $img=$(`#p_${id}`,wrap), $file=$(`#f_${id}`,wrap), $def=$(`#d_${id}`,wrap);
      async function updPrev(){ const u=sanitizeUrl($inp.value); if(!u){ $img.src=''; return; } const r=await preloadImg(u); if(r.ok){ $img.src=u; } else { $img.src=''; } }
      if(val) updPrev();
      $inp.onchange=()=>{ const u=sanitizeUrl($inp.value); if(!u){ alert('URL muss http(s) oder /assets/img/... sein'); return; } settings.assets=settings.assets||{}; settings.assets.rightImages=settings.assets.rightImages||{}; settings.assets.rightImages[name]=u; updPrev(); };
      $file.onchange=()=> uploadGeneric($file, (p)=>{ $inp.value=p; settings.assets=settings.assets||{}; settings.assets.rightImages=settings.assets.rightImages||{}; settings.assets.rightImages[name]=p; updPrev(); });
      $def.onclick=()=>{ $inp.value=DEFAULTS.assets.rightImage; settings.assets=settings.assets||{}; settings.assets.rightImages=settings.assets.rightImages||{}; settings.assets.rightImages[name]=DEFAULTS.assets.rightImage; updPrev(); };
      return wrap;
    }
    function renderRightImgs(){ const host=$('#rightImgs'); host.innerHTML=''; settings.assets=settings.assets||{}; settings.assets.rightImages=settings.assets.rightImages||{}; (schedule.saunas||[]).forEach(name=> host.appendChild(rightImgRow(name, settings.assets.rightImages[name]||'')) ); }
    ...
    function collectSettingsFromUI(){
      const body={
        ...
        settings: {
          ...
          fonts: {...(settings.fonts||{}), scale: asInt($('#fontScale').value,1)},
          slides: {
            ...settings.slides,
            overviewDurationSec: asInt($('#overviewSec').value,DEFAULTS.slides.overviewDurationSec),
            saunaDurationSec:    asInt($('#saunaSec').value,DEFAULTS.slides.saunaDurationSec),
            transitionMs:        asInt($('#transMs').value,DEFAULTS.slides.transitionMs),
            tileWidthPercent:    asInt($('#tileW').value,DEFAULTS.slides.tileWidthPercent),
            tileMinPx:           asInt($('#tileMin').value,DEFAULTS.slides.tileMinPx),
            tileMaxPx:           asInt($('#tileMax').value,DEFAULTS.slides.tileMaxPx)
          },
          display: {
            ...(settings.display||{}),
            fit: $('#fitMode').value||DEFAULTS.display.fit,
            rightWidthPercent: asInt($('#rightW').value,DEFAULTS.display.rightWidthPercent),
            cutTopPercent:     asInt($('#cutTop').value,DEFAULTS.display.cutTopPercent),
            cutBottomPercent:  asInt($('#cutBottom').value,DEFAULTS.display.cutBottomPercent)
          },
          theme: {...collectColors()},
          assets: { ...(settings.assets||{}), flameImage: sanitizeUrl($('#flameImg').value) || DEFAULTS.assets.flameImage },
          highlightNext: {
            enabled: $('#hlEnabled').checked,
            color: /^#([0-9A-Fa-f]{6})$/.test($('#hlColor').value)? $('#hlColor').value.toUpperCase() : DEFAULTS.highlightNext.color,
            minutesAfter: asInt($('#hlAfter').value, DEFAULTS.highlightNext.minutesAfter)
          }
        }
      };
      return body;
    ...
    $('#btnSave').onclick=async()=>{
      // refresh rightImages from inputs
      settings.assets=settings.assets||{}; settings.assets.rightImages=settings.assets.rightImages||{};
      $$('#rightImgs input[type="url"]').forEach(inp=>{ const label=inp.parentElement.parentElement.querySelector('label').textContent; const u=sanitizeUrl(inp.value); settings.assets.rightImages[label]= u || DEFAULTS.assets.rightImage; });
      ...
      const j=await r.json(); alert(j.ok? 'Gespeichert.' : ('Fehler: '+(j.error||'unbekannt')));
    };

    // Vorschau: iframe + postMessage
    ...
    function sendPreview(reload){ const f=$('#prevFrame'); if(reload){ f.contentWindow.location.reload(); setTimeout(()=>sendPreview(),450); return; } const payload=collectSettingsFromUI(); setTimeout(()=>{ f.contentWindow.postMessage({type:'preview', payload}, '*'); }, 400); }
    ...
    // Assets Cleanup
    $('#btnCleanup').onclick=async()=>{ if(!confirm('Unbenutzte Bilder in /assets/img/ löschen? (Defaults & aktuell verwendete bleiben erhalten)')) return; const r=await fetch('/admin/api/cleanup_assets.php',{method:'POST'}); const j=await r.json(); alert(j.ok? ('Gelöscht: '+(j.removed||[]).length+' Dateien') : ('Fehler: '+(j.error||'unbekannt'))); };
