...
trimmed AS (
    WITH RECURSIVE shrink(history, remaining, needs_trim) AS (
        SELECT appended,
               MAX(json_array_length(appended) - :historyLimit, 0),
               CASE WHEN json_array_length(appended) > :historyLimit THEN 1 ELSE 0 END
        FROM prepared
        UNION ALL
        SELECT json_remove(history, '$[0]'), remaining - 1, needs_trim
        FROM shrink
        WHERE needs_trim = 1 AND remaining > 0
    )
