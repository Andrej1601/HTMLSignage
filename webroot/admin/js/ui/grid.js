...
const CONFLICT_SEVERITY_ORDER = { warn: 1, crit: 2 };
let conflictPanelExpanded = false;
let lastConflictData = {
  conflicts: [],
  timeSeverity: new Map(),
  cellSeverity: new Map()
};
...
function setConflictSeverity(map, key, severity){
  if (!map || key == null || !severity) return;
  const current = map.get(key);
  const currentRank = current ? (CONFLICT_SEVERITY_ORDER[current] || 0) : 0;
  const nextRank = CONFLICT_SEVERITY_ORDER[severity] || 0;
...
function computeGridConflicts(schedule){
  const result = {
    conflicts: [],
    timeSeverity: new Map(),
    cellSeverity: new Map()
  ...
  const rowInfo = rows.map((row) => {
    ...
    const normalizedTime = rawTime ? parseTime(rawTime) : '';
    return { rawTime, normalizedTime: normalizedTime || null };
  });
  ...
  rowInfo.forEach((info, rowIdx) => {
    if (!info.rawTime){
      setConflictSeverity(result.timeSeverity, rowIdx, 'warn');
      result.conflicts.push({
        id: `time-missing-${rowIdx}`,
        severity: 'warn',
        title: 'Zeit fehlt',
        description: `Zeile ${rowIdx + 1} enthält keine Uhrzeit.`,
        meta: [`Zeile ${rowIdx + 1}`],
        focusTargets: [{ type: 'time', row: rowIdx, label: 'Zur Zeit' }]
      });
      return;
    }
    if (!info.normalizedTime){
      setConflictSeverity(result.timeSeverity, rowIdx, 'crit');
      result.conflicts.push({
        id: `time-invalid-${rowIdx}`,
        severity: 'crit',
        title: `Ungültige Uhrzeit (${info.rawTime})`,
        description: `Bitte Zeit im Format HH:MM für Zeile ${rowIdx + 1} angeben.`,
        meta: [`Zeile ${rowIdx + 1}`],
        focusTargets: [{ type: 'time', row: rowIdx, label: 'Zur Zeit' }]
      });
      return;
    }
    const minutes = timeToMinutes(info.normalizedTime);
    if (lastValid && minutes != null && lastValid.minutes != null && minutes < lastValid.minutes){
      setConflictSeverity(result.timeSeverity, rowIdx, 'warn');
      result.conflicts.push({
        id: `time-order-${rowIdx}`,
        severity: 'warn',
        title: 'Zeit außerhalb der Reihenfolge',
        description: `${info.normalizedTime} liegt vor ${lastValid.time}.`,
        meta: [`Zeile ${rowIdx + 1}`, `Vorher: Zeile ${lastValid.row + 1}`],
        focusTargets: [
          { type: 'time', row: lastValid.row, label: lastValid.time },
          { type: 'time', row: rowIdx, label: info.normalizedTime }
        ]
      });
    } else if (minutes != null) {
      lastValid = { minutes, time: info.normalizedTime, row: rowIdx };
    }
  });

  const duplicates = new Map();
  rows.forEach((row, rowIdx) => {
    const normalizedTime = rowInfo[rowIdx]?.normalizedTime;
    if (!normalizedTime) return;
    const entries = Array.isArray(row.entries) ? row.entries : [];
    entries.forEach((cell, colIdx) => {
      if (!cell || typeof cell !== 'object') return;
      const title = typeof cell.title === 'string' ? cell.title.trim() : '';
      if (!title) return;
      const key = `${colIdx}|${normalizedTime}`;
      if (!duplicates.has(key)) duplicates.set(key, []);
      duplicates.get(key).push({ rowIdx, colIdx });
    });
  });

  duplicates.forEach((list, key) => {
    if (!Array.isArray(list) || list.length <= 1) return;
    const [colIdxRaw, timeValue] = key.split('|');
    const colIdx = Number(colIdxRaw);
    const saunaNameRaw = typeof saunas[colIdx] === 'string' ? saunas[colIdx].trim() : '';
    const saunaName = saunaNameRaw || `Sauna ${colIdx + 1}`;
    list.forEach(({ rowIdx, colIdx: column }) => {
      setConflictSeverity(result.cellSeverity, `${rowIdx}:${column}`, 'crit');
      setConflictSeverity(result.timeSeverity, rowIdx, 'crit');
    });
    result.conflicts.push({
      id: `duplicate-${colIdx}-${timeValue}`,
      severity: 'crit',
      title: `Doppelbelegung in „${saunaName}“`,
      description: `Für ${timeValue} Uhr sind ${list.length} Einträge in derselben Sauna geplant.`,
      meta: [
        `Sauna: ${saunaName}`,
        `Zeit: ${timeValue}`,
        `Zeilen: ${list.map(({ rowIdx }) => rowIdx + 1).join(', ')}`
      ],
      focusTargets: list.slice(0, 4).map(({ rowIdx, colIdx: column }) => ({
        type: 'cell',
        row: rowIdx,
        col: column,
        label: `Zeile ${rowIdx + 1}`
      }))
    });
  });

  result.conflicts.sort((a, b) => {
    const rankDiff = (CONFLICT_SEVERITY_ORDER[b.severity] || 0) - (CONFLICT_SEVERITY_ORDER[a.severity] || 0);
...
function applyGridConflicts(data){
  const table = $('#grid');
  if (!table) return;
  const timeSeverity = data?.timeSeverity instanceof Map ? data.timeSeverity : new Map();
  const cellSeverity = data?.cellSeverity instanceof Map ? data.cellSeverity : new Map();

  table.querySelectorAll('.time[data-ri]').forEach((cell) => {
    const rowIdx = Number(cell.dataset.ri);
    const severity = timeSeverity.get(rowIdx);
    if (severity) cell.setAttribute('data-conflict', severity);
    else cell.removeAttribute('data-conflict');
  });

  table.querySelectorAll('.cellbtn').forEach((btn) => {
    const rowIdx = Number(btn.dataset.ri);
    const colIdx = Number(btn.dataset.ci);
    const severity = cellSeverity.get(`${rowIdx}:${colIdx}`);
    if (severity) btn.setAttribute('data-conflict', severity);
...
function renderConflictPanel(data){
  ...
  const summaryEl = $('#conflictPanelSummary');
  const toggle = $('#conflictPanelToggle');
  const body = $('#conflictPanelBody');
  ...
  if (!conflicts.length){
    ...
    if (toggle){
      ...
      toggle.textContent = 'Bericht anzeigen';
    }
    if (summaryEl){
      summaryEl.textContent = 'Keine Konflikte';
    }
    if (body){
      body.innerHTML = '';
      body.hidden = true;
  ...
  if (summaryEl){
    const critCount = conflicts.filter((entry) => entry.severity === 'crit').length;
    const warnCount = conflicts.filter((entry) => entry.severity === 'warn').length;
    const labelParts = [];
    if (critCount) labelParts.push(`${critCount} kritisch`);
    if (warnCount) labelParts.push(`${warnCount} Hinweis${warnCount === 1 ? '' : 'e'}`);
    if (!labelParts.length) labelParts.push(`${conflicts.length} Konflikt${conflicts.length === 1 ? '' : 'e'}`);
    const fragment = document.createDocumentFragment();
    const titleSpan = document.createElement('span');
    titleSpan.textContent = 'Konfliktprüfung';
    fragment.appendChild(titleSpan);
    const countSpan = document.createElement('span');
    countSpan.className = 'conflict-count';
    if (critCount) countSpan.classList.add('crit');
    else if (warnCount) countSpan.classList.add('warn');
    countSpan.textContent = labelParts.join(' · ');
  ...
  if (toggle){
    ...
    toggle.setAttribute('aria-expanded', conflictPanelExpanded ? 'true' : 'false');
    toggle.textContent = conflictPanelExpanded ? 'Bericht ausblenden' : 'Bericht anzeigen';
  }

  if (body){
    body.hidden = !conflictPanelExpanded;
    body.innerHTML = '';
    ...
    conflicts.forEach((entry) => {
      ...
      if (entry.severity) item.dataset.severity = entry.severity;
      const title = document.createElement('div');
      title.className = 'conflict-item-title';
      title.textContent = entry.title || 'Konflikt';
      ...
      if (Array.isArray(entry.focusTargets)){
        entry.focusTargets.forEach((target) => {
          if (!target || !target.type) return;
          const btn = document.createElement('button');
          btn.type = 'button';
          btn.className = 'btn xs ghost';
          btn.dataset.conflictFocus = target.type;
          if (target.row != null) btn.dataset.row = String(target.row);
          if (target.col != null) btn.dataset.col = String(target.col);
          btn.textContent = target.label || 'Springen';
...
function setConflictPanelExpanded(flag){
  conflictPanelExpanded = !!flag;
}

function clearConflictFocus(){
  document.querySelectorAll('#grid .conflict-focus').forEach((node) => node.classList.remove('conflict-focus'));
}

function focusConflictTarget(target){
  if (!target || !target.type) return;
  const table = $('#grid');
  if (!table) return;
  clearConflictFocus();
  if (target.type === 'time'){
    const rowIdx = Number(target.row);
    if (!Number.isFinite(rowIdx)) return;
    const timeCell = table.querySelector(`.time[data-ri="${rowIdx}"]`);
    if (!timeCell) return;
    timeCell.classList.add('conflict-focus');
    const input = timeCell.querySelector('.time-input');
    if (input){
      if (typeof input.focus === 'function') input.focus({ preventScroll: true });
      if (typeof input.select === 'function') input.select();
    }
    if (typeof timeCell.scrollIntoView === 'function'){
      timeCell.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  } else if (target.type === 'cell'){
    const rowIdx = Number(target.row);
    const colIdx = Number(target.col);
    if (!Number.isFinite(rowIdx) || !Number.isFinite(colIdx)) return;
    const btn = table.querySelector(`.cellbtn[data-ri="${rowIdx}"][data-ci="${colIdx}"]`);
    if (!btn) return;
    btn.classList.add('conflict-focus');
...
export function renderGrid(){
  ...
  const conflictData = computeGridConflicts(sc);
  lastConflictData = conflictData;
  applyGridConflicts(conflictData);
  renderConflictPanel(conflictData);
...
function initOnce(){
  ...
  const conflictToggle = document.getElementById('conflictPanelToggle');
  if (conflictToggle && !conflictToggle.dataset.bound) {
    conflictToggle.dataset.bound = '1';
    conflictToggle.addEventListener('click', () => {
      if (!lastConflictData?.conflicts?.length) return;
      setConflictPanelExpanded(!conflictPanelExpanded);
      renderConflictPanel(lastConflictData);
    });
  }
  const conflictBody = document.getElementById('conflictPanelBody');
  if (conflictBody && !conflictBody.dataset.bound) {
    conflictBody.dataset.bound = '1';
    conflictBody.addEventListener('click', (event) => {
      const btn = event.target.closest('[data-conflict-focus]');
      if (!btn) return;
      event.preventDefault();
      const type = btn.dataset.conflictFocus;
      const row = btn.dataset.row !== undefined && btn.dataset.row !== '' ? Number(btn.dataset.row) : null;
      const col = btn.dataset.col !== undefined && btn.dataset.col !== '' ? Number(btn.dataset.col) : null;
      focusConflictTarget({ type, row, col });
    });
  }
}
